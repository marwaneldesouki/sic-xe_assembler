import sys
import re
filename = f"G:\gam3a\System Programming\Lab\Project\\text.txt"
array = []
address = 0000
old_line = ""
with open(filename) as f:
    for line in f:
        ...
        if(line.split("\t")[1] == "START"): #start
            ...
            try:
                charRegex = re.findall("(.*?)'", old_line[2]) # get the char before '
                if(charRegex[0] == "c"):
        ...
        line += "\t"+str(hex(int(address))[2:].zfill(4)) # convert address to hex
        line = line.replace("\n", "").replace(
            ".", "").replace("â€™", "").split("\t")
        old_line= line
        print(line)
